...
for (( i = 0; i < $exec_count; i++)); do
    sudo python3 $IOTRACER_PATH -t Thread-,conn,java,mongod -l b $block_device -s $storage_device > traces_ycsb/storage/trace_ycsb_storage_disk_$i &
    sleep 4
    sudo sync; echo 3 > /proc/sys/vm/drop_caches 
...
for (( i = 0; i < $exec_count; i++)); do
    sudo python3 $IOTRACER_PATH -t Thread-,conn,java,mongod -l b $block_device -s $storage_device > /tmp/trace_ycsb_storage_ram_$i &
    sleep 4
    sudo sync; echo 3 > /proc/sys/vm/drop_caches 
