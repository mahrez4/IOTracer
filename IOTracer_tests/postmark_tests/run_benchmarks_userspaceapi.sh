...
for (( i = 0; i < $exec_count; i++)); do
    sudo python3 $IOTRACER_PATH -t postmark --dir -i $inode -l b $block_device -u $userspace_api -sleep 1 > traces_postmark/userspace_api/trace_postmark_userspace_consume-sleep 1s_$i &
    sleep 4
    sudo sync; echo 3 > /proc/sys/vm/drop_caches 
...
for (( i = 0; i < $exec_count; i++)); do
    sudo python3 $IOTRACER_PATH -t postmark --dir -i $inode -l b $block_device -u $userspace_api -sleep 1 > traces_postmark/userspace_api/trace_postmark_userspace_poll-sleep 1s_$i &
    sleep 4
    sudo sync; echo 3 > /proc/sys/vm/drop_caches 
