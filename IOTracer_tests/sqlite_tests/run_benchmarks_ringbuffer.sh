...
for (( i = 0; i < $exec_count; i++)); do
    sudo python3 $IOTRACER_PATH -t sqlite --dir -i $inode -l s -size $ringbuf_size > traces_sqlite/ringbuffer/trace_sqlite_ringbuf_128kb_$i &
    sleep 4
    sudo sync; echo 3 > /proc/sys/vm/drop_caches 
...
for (( i = 0; i < $exec_count; i++)); do
    sudo python3 $IOTRACER_PATH -t sqlite --dir -i $inode -l s -size $ringbuf_size > traces_sqlite/ringbuffer/trace_sqlite_ringbuf_4mb_$i &
    sleep 4
    sudo sync; echo 3 > /proc/sys/vm/drop_caches 
...
for (( i = 0; i < $exec_count; i++)); do
    sudo python3 $IOTRACER_PATH -t sqlite --dir -i $inode -l s -size $ringbuf_size > traces_sqlite/ringbuffer/trace_sqlite_ringbuf_128mb_$i &
    sleep 4
    sudo sync; echo 3 > /proc/sys/vm/drop_caches 
...
for (( i = 0; i < $exec_count; i++)); do
    sudo python3 $IOTRACER_PATH -t sqlite --dir -i $inode -l s -size $ringbuf_size > traces_sqlite/ringbuffer/trace_sqlite_ringbuf_1G_$i &
    sleep 4
    sudo sync; echo 3 > /proc/sys/vm/drop_caches 
