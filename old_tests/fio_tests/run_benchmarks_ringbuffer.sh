...
sudo python $IOTRACER_PATH -t fio --file -i $inode -l b -size $ringbuf_size > trace_output_bcc &
sleep 5
...
rm fio_results_ringbuf_1G

for (( i = 0; i < 20; i++)); do
    sudo sync; echo 3 > /proc/sys/vm/drop_caches 
    fio $fio_config >> fio_results_ringbuf_1G
    echo \n------------------------------------------\n >> fio_results_ringbuf_1G
done    
